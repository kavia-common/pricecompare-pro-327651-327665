...
class GameNationScraper:
    site_code = "gamenation"
    ...
    async def search(self, query: str, ctx: ScrapeContext) -> List[NormalizedOffer]:
        ...
        for a in soup.select("a[href*='/products/'], a.product-card, a.productitem--title"):
            href = a.get("href")
            if not href:
                continue
            ...
            if container:
                price_el = container.select_one(".price, .money, .productitem--price")
                if price_el:
    ...
    async def from_url(self, url: str, ctx: ScrapeContext) -> List[NormalizedOffer]:
        ...
        price_el = soup.select_one(".price, .money")
        price_text = price_el.get_text(" ", strip=True) if price_el else None

        img_el = soup.select_one("img.product__image, img")
